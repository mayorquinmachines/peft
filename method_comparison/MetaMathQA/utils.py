...
class TrainConfig:
    ...
    Args:
        ...
        optimizer_type: The name of a torch optimizer (e.g. AdamW) or a PEFT method ("lora+", "lora-fa", "bidora")
    ...
    def __post_init__(self) -> None:
        ...
        if self.optimizer_type not in ["lora+", "lora-fa", "bidora"] and not hasattr(torch.optim, self.optimizer_type):
...
def get_optimizer_and_scheduler(
    ...
    elif optimizer_type == "bidora":
        # BiDoRA-style bi-level optimization uses a normal optimizer; the alternating updates of the magnitude and
        # direction components are handled in the training loop (see bilevel_optimization.py).
