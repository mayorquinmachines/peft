...
class LoraConfig(PeftConfig):
    ...
    Args:
        ...
        init_lora_weights (`bool` | `Literal["gaussian", "eva", "osrm", "olora", "pissa", "pissa_niter_[number of iters]", "corda", "loftq", "orthogonal", "mica"]`):
            ...
            performance due to its ability to adapt to the finetuning data. Passing `'osrm'` results in a data-driven
            initialization of <a href='https://huggingface.co/papers/2505.22934' >Orthogonal Subspaces for Robust Model
            Merging</a>. OSRM initializes LoRA A into the low-interference subspace of out-of-task layer input
            activations (the eigenvectors of their covariance with the smallest eigenvalues), which reduces cross-task
            interference when multiple LoRA adapters are merged. Pass `'olora'` to use OLoRA initialization.
    ...
    init_lora_weights: (
        ...
        metadata={
            "help": (
                ...
                "Passing `'osrm'` results in a data-driven initialization of Orthogonal Subspaces for Robust Model "
                "Merging, where LoRA A is initialized into the low-interference subspace of out-of-task layer input "
                "activations to reduce cross-task interference when merging adapters. "
